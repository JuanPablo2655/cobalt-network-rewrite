...
export async function deposit(cobalt: CobaltClient, interaction: ChatInputCommandInteraction<'cached'>) {
	const profile = await cobalt.container.db.getUser(interaction.user.id);
	...
	await cobalt.container.econ.addToBank(interaction.user.id, amount);
...
export async function withdraw(cobalt: CobaltClient, interaction: ChatInputCommandInteraction<'cached'>) {
	...
	await cobalt.container.econ.removeFrombank(interaction.user.id, amount);
	await cobalt.container.econ.addToWallet(interaction.user.id, amount);
	return interaction.reply({
