...
export async function deposit(cobalt: CobaltClient, interaction: CommandInteraction) {
	...
	if ((profile?.wallet ?? 0) - amount <= 0) return interaction.reply({ content: "You don't have that much money" });
	if ((profile?.bank ?? 0) + amount > (profile?.bankSpace ?? 0))
