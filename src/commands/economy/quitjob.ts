...
abstract class QuitJobCommand extends GenericCommand {
    ...
    async run(message: Message, _args: string[], addCD: Function) {
        ...
        if (user?.job === null) return message.reply(`You don't have a job to quit from.`)
        await this.cobalt.econ.updateJob(message.author.id, null);
        return message.channel.send(`You have successfully quit from your current job. You need to apply to a new job if you want to work.`);
    };
};
