	...
	async run(message: Message, args: string[], addCD: Function) {
		...
		if (member?.id === message.author.id) {
			...
			return message.channel.send({
				content: `You have received your weekly **₡${this.cobalt.utils.formatNumber(weeklyAmount)}**.`,
			});
		}
		...
		return message.channel.send({
			content: `You gave your weekly of **₡${this.cobalt.utils.formatNumber(moneyEarned)}** to **${
