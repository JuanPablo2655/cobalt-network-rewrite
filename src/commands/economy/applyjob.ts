...
abstract class ApplyJobCommand extends GenericCommand {
    ...
    async run(message: Message, args: string[], addCD: Function) {
        ...
        if (!args[0]) return message.reply(`Please provide a job id. You can find a list by running \`${guild?.prefix}listjobs\``);
        ...
        if (user?.job === null) {
            ...
            return message.reply(`Congraduations on becoming a **${job.name}**. Your minimum payment is now **₡${this.cobalt.utils.formatNumber(job.minAmount)}**`);
        } else {
