...
abstract class DepositCommand extends GenericCommand {
    ...
    async run(message: Message, args: string[], addCD: Function) {
        ...
        return message.channel.send(`You deposited **₡${this.cobalt.utils.formatNumber(money)}**. Your bank balance is now **₡${this.cobalt.utils.formatNumber(profile!.bank + money)}**`);
    };
};
