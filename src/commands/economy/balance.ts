...
abstract class BalanceCommand extends GenericCommand {
    ...
    async run(message: Message, args: string[], addCD: Function) {
        ...
        const balanceEmbed = new MessageEmbed()
            ...
            .setDescription(`**Wallet**: ₡${this.cobalt.utils.formatNumber(profile!.wallet)}\n**Bank**: ₡${this.cobalt.utils.formatNumber(profile!.bank)} / ₡${this.cobalt.utils.formatNumber(profile!.bankSpace)} \`${bankPercent.toString().substring(0, 4)}%\`\n**Net Worth**: ₡${this.cobalt.utils.formatNumber(profile!.netWorth)}\n**Bounty**: ₡${this.cobalt.utils.formatNumber(profile!.bounty)}`)
