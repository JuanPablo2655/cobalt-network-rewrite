...
abstract class WithdrawCommand extends GenericCommand {
    ...
    async run(message: Message, args: string[], addCD: Function) {
        ...
        return message.channel.send(`You withdrew **₡${this.cobalt.utils.formatNumber(money)}**. Your bank balance is now **₡${this.cobalt.utils.formatNumber(profile!.bank - money)}**`);
    };
};
